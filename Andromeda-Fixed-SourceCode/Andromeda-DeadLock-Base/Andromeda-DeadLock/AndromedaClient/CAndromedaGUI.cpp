...
void CAndromedaGUI::OnInit(IDXGISwapChain* pSwapChain)
{
...
void CAndromedaGUI::InitFont()
{
	...
	m_pFontAwesomeIcons = io.Fonts->AddFontFromMemoryCompressedTTF(FontAwesomeIcon_compressed_data, FontAwesomeIcon_compressed_size, 25.f,
	                                                               &FontAwesomeIconConfig, AwesomeIconRanges);

	CoTaskMemFree(szWindowsFontPath);
}

void CAndromedaGUI::InitIndigoStyle()
{
...
void CAndromedaGUI::InitVermillionStyle()
{
	auto& style = ImGui::GetStyle();
	auto& colors = style.Colors;
...
void CAndromedaGUI::InitClassicSteamStyle()
{
...
void CAndromedaGUI::UpdateStyle()
{
	ImGui::SetCurrentContext(m_pImGuiContext);

	if (GetMisc()->MenuStyle == EAndromedaGuiStyle::INDIGO)
		InitIndigoStyle();
	else if (GetMisc()->MenuStyle == EAndromedaGuiStyle::VERMILLION)
		InitVermillionStyle();
	else if (GetMisc()->MenuStyle == EAndromedaGuiStyle::CLASSIC_STEAM)
...
void CAndromedaGUI::OnRender(IDXGISwapChain* pSwapChain)
{
	...
	else
	{
...

		{
			...
			{
				const ImVec2 vBB{(float)scDesc.BufferDesc.Width, (float)scDesc.BufferDesc.Height};
				if (vBB.x > 0.f && vBB.y > 0.f)
				{
					m_vViewportSize = vBB;
					ImGui::GetIO().DisplaySize = vBB;
				}
			}
		}
...
void CAndromedaGUI::OnReopenGUI()
{
...
void CAndromedaGUI::ClearRenderTargetView()
{
	if (m_pRenderTargetView)
	{
