...
namespace Math
{
    auto WorldToScreen( const Vector3& vIn , ImVec2& vOut ) -> bool
    {
        ...
        auto ret = !( ScreenTransform( vIn , Out ) );

        const auto& io = ImGui::GetIO();
        vOut.x = ( ( Out.m_x + 1.0f ) * 0.5f ) * io.DisplaySize.x;
        vOut.y = io.DisplaySize.y - ( ( ( Out.m_y + 1.0f ) * 0.5f ) * io.DisplaySize.y );
    ...
    auto WorldToScreen( const Vector3& vIn , Vector2& vOut ) -> bool
    {
        ...
        const auto& io = ImGui::GetIO();
        vOut.m_x = ( ( Out.m_x + 1.0f ) * 0.5f ) * io.DisplaySize.x;
        vOut.m_y = io.DisplaySize.y - ( ( ( Out.m_y + 1.0f ) * 0.5f ) * io.DisplaySize.y );
    ...
    auto WorldToScreen( const Vector3& vIn , Vector3& vOut ) -> bool
    {
        ...
        auto ret = !( ScreenTransform( vIn , vOut ) );

        const auto& io = ImGui::GetIO();
        vOut.m_x = ( ( vOut.m_x + 1.0f ) * 0.5f ) * io.DisplaySize.x;
        vOut.m_y = io.DisplaySize.y - ( ( ( vOut.m_y + 1.0f ) * 0.5f ) * io.DisplaySize.y );
