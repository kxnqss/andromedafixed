...
class CUserCmdArray
{
	// [D] client.dll pattern: 44 8B B0 ? ? ? ? 41 8B D6
	static constexpr uintptr_t k_nSequenceNumber = 0x6270;
public:
	CUSTOM_OFFSET_FIELD(uint32_t, m_nSequenceNumber, k_nSequenceNumber);
};
...
class CUserCmd
{
	...
	// Triggers a one-frame press: sets all three buttonstate fields.
	// buttonstate1 = held, buttonstate2 = pressed this frame, buttonstate3 = released this frame.
	// After this tick the engine resets them, so the button is "tapped" for one frame only.
	auto TapButton( InputBitMask_t mask )
	{
		this->button_states.buttonstate1 |= mask;
		this->button_states.buttonstate2 |= mask;
		this->button_states.buttonstate3 |= mask;

		if ( !this->cmd.has_base() )
			return;

		auto* pBase = this->cmd.mutable_base();
		if ( !pBase->has_buttons_pb() )
			return;

		auto* pButtons = pBase->mutable_buttons_pb();
		pButtons->set_buttonstate1( pButtons->buttonstate1() | mask );
		pButtons->set_buttonstate2( pButtons->buttonstate2() | mask );
		pButtons->set_buttonstate3( pButtons->buttonstate3() | mask );
	}

	// Returns true if the button is currently held (buttonstate1 set).
	bool IsButtonHeld( InputBitMask_t mask ) const
	{
		return ( button_states.buttonstate1 & mask ) != 0;
	}

	// Clears a button from all three buttonstate fields.
	// Call every frame to suppress a button the user may have pressed.
	auto ClearButton( InputBitMask_t mask )
	{
		this->button_states.buttonstate1 &= ~mask;
		this->button_states.buttonstate2 &= ~mask;
		this->button_states.buttonstate3 &= ~mask;
	...
	// Holds a button down: sets only buttonstate1.
	// Call every frame to keep the button held; the engine reads this as "still pressed".
	auto HoldButton( InputBitMask_t mask )
	{
		this->button_states.buttonstate1 |= mask;
