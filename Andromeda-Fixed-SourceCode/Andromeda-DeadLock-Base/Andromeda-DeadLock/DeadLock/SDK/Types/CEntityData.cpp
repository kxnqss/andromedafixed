...
auto C_BaseEntity::IsCitadelFissureWall() -> bool
{
	const auto* pszDesingerName = pEntityIdentity()->DesingerName().String();
	return pszDesingerName && strcmp(pszDesingerName, XorStr("citadel_fissure_wall")) == 0;
}

auto C_BaseEntity::IsCitadelTeam() -> bool
{
	auto* pBinding = GetSchemaClassBinding();
	if (!pBinding)
		return false;

	const auto* pszName = pBinding->m_bindingName();
	return pszName && strcmp(pszName, XorStr("C_CitadelTeam")) == 0;
}
...
auto C_BaseEntity::ComputeHitboxBounds(Vector3& mins, Vector3& maxs) -> bool
{
	return C_BaseEntity_ComputeHitboxBounds(this, mins, maxs);
}

auto C_BaseEntity::ComputeHitboxBounds(Rect_t& out) -> bool
{
	Vector3 mins;
	Vector3 maxs;

	if (!this->ComputeHitboxBounds(mins, maxs))
		return false;
...
auto C_BaseModelEntity::GetBoundingBox(Rect_t& out) -> bool
{
	...
	Vector3 mins = m_Collision().m_vecMins() + absOrigin;
	Vector3 maxs = m_Collision().m_vecMaxs() + absOrigin;
